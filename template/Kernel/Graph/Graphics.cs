...
namespace NativeOS.Graph
{
    public unsafe class Graphics
    {
        ...
        public virtual void Move(int dX, int dY, int sX, int sY, int Width, int Height)
        {
            for (int h = 0; h < Height; h++)
            {
                if (sY + h < 0) continue;
                var off1 = this.Width * (dY + h) + (dX);
                var off2 = this.Width * (sY + h) + (sX);
                Native.Movsb(VideoMemory + off1, VideoMemory + off2,(uint)Width * sizeof(uint));
            }
        }
        ...
        public virtual void DrawImage(int X, int Y, Image image, bool AlphaBlending = false)
        {
            if (AlphaBlending) 
            {
